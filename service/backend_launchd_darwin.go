...
import (
	...
	"github.com/hedzr/gsvc/service/systems"
...
func launchdRestart(ctx context.Context, config *Config, m *mgmtS, s *launchD) (err error) {
	...
	if dir.FileExists(file) {
		var msg string
		retCode, msg, err = exec.Sudo("launchctl", "restart", serviceName)
		if err != nil || retCode != 0 {
...
func launchdUninstall(ctx context.Context, config *Config, m *mgmtS, s *launchD) (err error) {
	...
	if dir.FileExists(file) {
		dbglog.WarnContext(ctx, "uninstalling service file: "+file)
		var msg string
		if userLevel {
